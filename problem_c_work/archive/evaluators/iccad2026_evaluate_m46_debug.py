...
def evaluate_solution(
    ...
    for i, (_, _, w, h) in enumerate(positions):
        if i >= len(target_areas) or target_areas[i] == -1:
            continue
        is_fixed = target_constraints is not None and target_constraints.shape[1] > 0 and target_constraints[i, 0] != 0
        is_preplaced = target_constraints is not None and target_constraints.shape[1] > 1 and target_constraints[i, 1] != 0
    ...
    if target_constraints is not None and len(target_constraints) >= block_count:
        ...
        boundary_violations = _check_boundary_constraints(positions, constraints)
        grouping_violations = _check_grouping_constraints(positions, constraints)
...
def score_saved_solutions(
    ...
    if debug_breakdown:
        csv_path = debug_csv_path or f"{Path(solutions_path).stem}_debug_breakdown.csv"
        write_debug_csv([r['cost_breakdown'] for r in results if r.get('cost_breakdown')], csv_path)
...
def main():
    ...
    if args.evaluate:
        ...
        if args.debug_breakdown:
            csv_path = args.debug_csv or f"{result.submission_name}_debug_breakdown.csv"
            write_debug_csv([r.cost_breakdown for r in result.test_results if r.cost_breakdown], csv_path)
            print(f"Debug CSV saved to {csv_path}")
