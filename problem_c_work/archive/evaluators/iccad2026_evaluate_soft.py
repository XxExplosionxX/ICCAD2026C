...
def positions_to_polygons(
    positions: List[Tuple[float, float, float, float]]
) -> List["Polygon"]:
    """Convert rectangle positions to shapely polygons."""
    if not SHAPELY_AVAILABLE:
        raise ImportError("shapely is required to compute soft constraints")

    return [box(x, y, x + w, y + h) for x, y, w, h in positions]


def target_blocks_to_polygons(
    polygons: torch.Tensor,
    block_count: int
) -> List["Polygon"]:
    """Convert dataset block polygons to shapely polygons."""
    if not SHAPELY_AVAILABLE:
        raise ImportError("shapely is required to compute soft constraints")
    ...
    for i in range(block_count):
        ...
        if len(valid) >= 3:
            coords = [(float(x), float(y)) for x, y in valid.tolist()]
            target_polygons.append(Polygon(coords))
        else:
            target_polygons.append(box(0, 0, 1, 1))
...
def evaluate_solution(
    ...
    target_polygons: Optional[List["Polygon"]] = None,
    ...
    if (
        SHAPELY_AVAILABLE and
        target_constraints is not None and
        len(target_constraints) >= block_count and
        target_polygons is not None
    ):
        pred_polygons = positions_to_polygons(positions)
        constraints = target_constraints[:block_count]

        if constraints.shape[1] > 0:
            fixed_indices = torch.where(constraints[:, 0] != 0)[0]
            if len(fixed_indices) > 0:
                fixed_violations = check_fixed_const(fixed_indices, pred_polygons, target_polygons)

        if constraints.shape[1] > 1:
            preplaced_indices = torch.where(constraints[:, 1] != 0)[0]
            if len(preplaced_indices) > 0:
        ...
        if constraints.shape[1] > 2:
            mib_violations = check_mib_const(constraints[:, 2], pred_polygons, target_polygons)

        if constraints.shape[1] > 3:
            grouping_violations = check_clust_const(constraints[:, 3], pred_polygons, target_polygons)

        if constraints.shape[1] > 4:
            bbox_width = max((x + w for x, _, w, _ in positions), default=0.0)
            bbox_height = max((y + h for _, y, _, h in positions), default=0.0)
            with contextlib.redirect_stdout(io.StringIO()):
                boundary_violations = check_boundary_const(
                    constraints[:, 4],
                    pred_polygons,
                    target_polygons,
                    max(1, int(math.ceil(bbox_width))),
                    max(1, int(math.ceil(bbox_height)))
                )
...
class ContestEvaluator:
    ...
    def _extract_baseline(self, idx, labels, b2b_conn, p2b_conn, pins_pos, block_count):
        ...
        target_polygons = target_blocks_to_polygons(polygons, block_count) if SHAPELY_AVAILABLE else None
        ...
        return {'hpwl_baseline': hpwl, 'area_baseline': area}, positions, target_polygons
    ...
    def evaluate(
        ...
        for idx in iterator:
            try:
                ...
                baseline, target_pos, target_polygons = self._extract_baseline(
