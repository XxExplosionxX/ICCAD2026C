#!/usr/bin/env python3
"""
M4c solver for ICCAD 2026 Problem C.

This is a stricter replan of M4:
- keeps exact portfolio branch selection
- only runs the expensive third branch when A and B are both poor and close
"""
...
class ContestOptimizer(contest.FloorplanOptimizer):
    ...
    def solve(
        ...
        base_positions = self._construct_layout(
            order,
            target_centers,
            shape_candidates,
            b2b_by_block,
        ...
        base_positions = self._compact(base_positions)

        candidates: List[List[Rect]] = [base_positions]

        full_refined = self._refine_layout(
            list(base_positions),
            target_centers,
            shape_candidates,
            degrees,
        ...
        full_refined = self._compact(full_refined)
        candidates.append(full_refined)

        metrics_a = self.query_cost(base_positions, b2b_connectivity, p2b_connectivity, pins_pos)
        metrics_b = self.query_cost(full_refined, b2b_connectivity, p2b_connectivity, pins_pos)

        if self._should_run_light_branch(block_count, base_positions, areas, metrics_a, metrics_b):
            light_refined = self._refine_layout_light(
                list(base_positions),
                target_centers,
                shape_candidates,
                degrees,
            ...
            light_refined = self._compact(light_refined)
            candidates.append(light_refined)

        return self._select_best_candidate(candidates, b2b_connectivity, p2b_connectivity, pins_pos)

    def _should_run_light_branch(
        self,
        block_count: int,
        base_positions: Sequence[Rect],
        areas: Sequence[float],
        metrics_a: Dict[str, float],
        metrics_b: Dict[str, float],
    ) -> bool:
        total_area = sum(areas)
        bbox_ratio = metrics_a["bbox_area"] / max(total_area, 1e-6)
        closeness = metrics_b["total"] / max(metrics_a["total"], 1e-6)
        if block_count <= 40:
            return bbox_ratio > 1.30 and closeness > 0.97
        if block_count <= 80:
            return bbox_ratio > 1.36 and closeness > 0.98
        return bbox_ratio > 1.42 and closeness > 0.985

    def _select_best_candidate(
        self,
        candidates: Sequence[Sequence[Rect]],
        b2b_connectivity: torch.Tensor,
        p2b_connectivity: torch.Tensor,
        pins_pos: torch.Tensor,
    ) -> List[Rect]:
        scored: List[Tuple[Tuple[float, float, float, float], List[Rect]]] = []
        for candidate in candidates:
            metrics = self.query_cost(list(candidate), b2b_connectivity, p2b_connectivity, pins_pos)
            key = (
                float(metrics["overlaps"]),
                float(metrics["hpwl_total"]),
                float(metrics["bbox_area"]),
                float(metrics["total"]),
            )
            scored.append((key, list(candidate)))
    ...
    def _refine_layout_light(
        ...
        for _ in range(rounds):
            block_order = sorted(
                range(n),
                key=lambda idx: (-self._local_cost(idx, current, b2b_by_block, p2b_by_block), -degrees[idx], idx),
            )
            improved_any = False
            for block_id in block_order[:blocks_per_round]:
                candidate = self._refine_block(
