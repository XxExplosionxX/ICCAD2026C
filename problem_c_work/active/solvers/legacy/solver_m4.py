#!/usr/bin/env python3
"""
M4 solver for ICCAD 2026 Problem C.

This is an isolated evolution of M2:
- keeps the stable M2 constructive flow
- generates a small deterministic portfolio of complete legal layouts
- selects the winning branch using exact full-layout query_cost metrics
"""
...
class ContestOptimizer(contest.FloorplanOptimizer):
    ...
    def _should_run_light_branch(
        ...
        if block_count <= 40:
            return True
        if block_count <= 80:
            return bbox_ratio > 1.34 or metrics_b["hpwl_total"] >= metrics_a["hpwl_total"] * 0.94
        return bbox_ratio > 1.38 or metrics_b["hpwl_total"] >= metrics_a["hpwl_total"] * 0.92
