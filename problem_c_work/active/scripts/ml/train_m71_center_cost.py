#!/usr/bin/env python3
"""
M7.1 training script:
- batched center prediction
- supervised center loss
- differentiable contest cost term
"""
...
from ml.m71_center_batch_model import M71CenterBatchModel


def _build_target_centers(fp_sol: torch.Tensor, scale_xy: torch.Tensor, valid_mask: torch.Tensor, device: torch.device) -> torch.Tensor:
    w = fp_sol[:, :, 0]
    h = fp_sol[:, :, 1]
    x = fp_sol[:, :, 2]
    y = fp_sol[:, :, 3]
    ...
    return target.to(device) * valid_mask.unsqueeze(-1).to(target.dtype)


def _build_positions_from_pred(pred_norm: torch.Tensor, scale_xy: torch.Tensor, fp_sol: torch.Tensor) -> torch.Tensor:
    w = fp_sol[:, :, 0]
    h = fp_sol[:, :, 1]
    cx = pred_norm[:, :, 0] * scale_xy[:, 0].unsqueeze(1)
    cy = pred_norm[:, :, 1] * scale_xy[:, 1].unsqueeze(1)
    x = cx - 0.5 * w
    y = cy - 0.5 * h
...
def _masked_center_loss(pred_norm: torch.Tensor, target_norm: torch.Tensor, valid_mask: torch.Tensor) -> torch.Tensor:
    diff = nn.functional.smooth_l1_loss(pred_norm, target_norm, reduction="none")
...
def _run_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        with amp_context:
            pred_norm, scale_xy, valid_mask = model(area_target, b2b_conn, p2b_conn, pins_pos, constraints)
            target_norm = _build_target_centers(fp_sol, scale_xy, valid_mask, device)
            center_loss = _masked_center_loss(pred_norm, target_norm, valid_mask)
            pred_positions = _build_positions_from_pred(pred_norm, scale_xy, fp_sol)

            cost_terms = []
            bsz = area_target.shape[0]
            for sample_idx in range(bsz):
                block_count = int(valid_mask[sample_idx].sum().item())
                positions = pred_positions[sample_idx, :block_count]
...
def main() -> None:
    ...
    parser.add_argument("--run-name", default="m71_center_cost")
    ...
    model = M71CenterBatchModel(hidden_dim=args.hidden_dim, depth=args.depth, dropout=args.dropout).to(device)
